...
class EuclideanDistTracker:
    def __init__(self):
        ...
        # {(class,id):(x,y,w,h,in_out,barcode,given)}
    ...
    def update_info(self, class_id, frame, bbox, objects_bbs_cids, in_out):
        ...
        if class_id[0] == 0:  # 바코드 일때
            ...
            objects_bbs_cids.append(
                [x, y, w, h, class_id, in_out, barcode_text])
    ...
    def update(self, classes, bboxes, frame, line):
        ...
        # given 을 false로 초기화
        # 밑에서 할당/업데이트 되면 True로 설정?

        for class_id, bbox_tmp in self.all_center_points.items():

            x, y, w, h, in_out, barcode,_ = bbox_tmp
            self.all_center_points[class_id]=(x, y, w, h, in_out, barcode,False)
        ...
        for one_class, bbox in zip(classes, bboxes):
            ...
            for class_id, bbox_tmp in self.all_center_points.items():
                x_tmp, y_tmp, w_tmp, h_tmp, in_out_tmp, barcode_tmp, given_tmp = bbox_tmp
                ...
                if one_class == class_id[0] and given_tmp == False:
                    ...
                    if one_class == 0: # barcode 인 경우
                        dist_standard, area_standard = 60, 10000
                        #print(f"barcode ::: dist = {dist}, area = {area}, dot1 = {dot1}, dot2 = {dot2}")

                    else:
                        dist_standard, area_standard = 60, 30000
                        #print(f"product ::: dist = {dist}, area = {area}, dot1 = {dot1}, dot2 = {dot2}")

                    # 거리가 25이하이면 id 객체 중심 위치 업데이트
                    if dist < dist_standard and abs(w_h_rate - w_h_rate_tmp)<ERROR : 
                        #print(f"{class_id}  dist = {dist}, area = {area}, dot1 = {dot1}, dot2 = {dot2}")
                        ...
                        self.all_center_points[class_id] = (
                            x, y, w, h, in_out, barcode,True)
                        objects_bbs_cids.append(
                            [x, y, w, h, class_id, in_out, barcode])
                        # 기존에 있던 객체임을 표시
                        same_object = True
                        break
                    else:  pass #print(f"dist = {dist}, area = {area}, dot1 = {dot1}, dot2 = {dot2}")
